...
TOOL_DEFS = {
    "shell_exec": _tool(
        "shell_exec", "Run a shell command in the project. Never destructive or network-mutating.",
        {"command": {"type": "string"}}, ["command"]),
    "read_file": _tool("read_file", "Read a file.", {"path": {"type": "string"}}, ["path"]),
    "write_file": _tool("write_file", "Create or overwrite a file (scope-checked).", {"path": {"type": "string"}, "content": {"type": "string"}}, ["path", "content"]),
    "append_file": _tool("append_file", "Append to a file (scope-checked).", {"path": {"type": "string"}, "content": {"type": "string"}}, ["path", "content"]),
    "code_edit": _tool("code_edit", "Find-and-replace in a file (scope-checked).", {"path": {"type": "string"}, "old_text": {"type": "string"}, "new_text": {"type": "string"}}, ["path", "old_text", "new_text"]),
    "list_directory": _tool("list_directory", "List a directory.", {"path": {"type": "string", "default": "."}}, []),
    "search_files": _tool("search_files", "Grep a directory.", {"query": {"type": "string"}, "path": {"type": "string", "default": "."}}, ["query"]),
    "web_search": _tool("web_search", "Web search via DuckDuckGo.", {"query": {"type": "string"}}, ["query"]),
    "web_fetch": _tool("web_fetch", "Fetch a URL as text.", {"url": {"type": "string"}}, ["url"]),
    "memory_store": _tool("memory_store", "Store a fact in project memory.", {"key": {"type": "string"}, "value": {"type": "string"}}, ["key", "value"]),
    "memory_recall": _tool("memory_recall", "Recall facts from project memory.", {"query": {"type": "string"}}, ["query"]),
    "finish": _tool("finish", "Finish the role with a structured result. Call with JSON.", {"result": {"type": "string"}}, ["result"]),
}
...
class ToolExecutor:
    ...
    def _check_write(self, path: str) -> Path:
        ...
        if not any(p.is_relative_to(a) for a in self.allowed_paths):
            raise ToolError(f"Path denied: {path} is outside allowed paths {[str(a) for a in self.allowed_paths]}")
        return p
    ...
    async def _shell(self, command: str) -> str:
        ...
        if bad:
            return f"Error: command blocked by policy ({bad})."
        ...
        return text[:50000] or "(no output)"
    ...
    def _read(self, path: str) -> str:
        ...
        return text[:100000] + ("\n… (truncated)" if len(text) > 100000 else "")
    ...
    def _write(self, name: str, args: dict) -> str:
        ...
        if name == "write_file":
            ...
            return f"Written {len(args.get('content', ''))} bytes → {p}"
        if name == "append_file":
            p.parent.mkdir(parents=True, exist_ok=True)
        ...
        if count != 1:
            return f"Error: old_text matched {count} times (need exactly 1)."
    ...
    def _list(self, path: str) -> str:
        ...
        for e in entries[:300]:
            lines.append(("[dir]  " if e.is_dir() else "[file] ") + e.name)
        return "\n".join(lines) or "(empty)"
    ...
    async def _search(self, query: str, path: str) -> str:
        ...
        if res.returncode not in (0, 1):
            return f"Error: search failed: {err.decode(errors='replace')[:500]}"
        return text[:100000].strip() or "No matches."
